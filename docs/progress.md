...
### 投递自检：把简历上的每个数字都变成可重跑的存档

`docs/resume.md` 的六条自检清单全部做完，顺带**查出两个真问题**。

**★问题一：开发库的 schema 落后了三个 stage。**
`runs` 表缺 `branch` / `pr_url` 两列（停在 Stage B 第三步之前），
于是**审批通过之后永远卡在 `publishing`** —— 发布循环每秒抛一次
`UndefinedColumnError` 并无限重试。

要命的是 **403 个测试全绿完全没发现它**：测试库靠 `conftest.py::_SCHEMA_MARKER`
自愈重建，开发库不会。
> **自愈机制盖住了它本该暴露的问题。** 一般化：**给开发期减摩擦的东西，
> 会顺手把"环境漂了"这个信号一起吃掉。** 所以光有测试不够，
> 必须有一条**端到端、打真实服务**的路径 —— 那才是唯一会撞上这种漂移的地方。
>
> 这也是「publisher 重试没有次数上限」那个已知缺口第一次真实发作：
> 一个必然失败的发布会永远重试下去，不会进终态。

**★问题二：沙箱探针的两组测的不是同一个路径。**
探针里写 `os.path.expanduser("~/.ssh")`，而容器里 `HOME=/tmp` ——
于是容器那组查的是 `/tmp/.ssh`，"看不见"当然看不见，它压根不存在。
> **对照实验的两组必须只差被测的那一个变量。** 路径这种会跟着环境漂的东西，
> 要在**宿主机**上算好再字面量塞进去。结论没变（4 项 → 0 项），
> 但当时那条证据是不成立的。

**新增 `scripts/demo_flow.sh`（`make demo-flow`）**：签名 webhook → 验签 → 幂等 →
clone → 容器沙箱 → 审批闸门 → 发布，**14 条断言，热启动 4 秒**，不花钱。
写它的时候还踩了自己一个坑：就绪循环只是 `curl ... && break`，
循环跑完照样往下走 —— **服务没起来却打印了「✓ 服务就绪」**，
后面所有断言拿到 000。**一个不会失败的检查比没有检查更糟，它把故障伪装成了通过。**
另外固定的 delivery-id 让演示只能跑一次（第二次全被判重）——
**可重复演示的前提是每次都用新的幂等键。**

**`docs/evidence/` 四份存档**，每份带重跑命令和花费：评测 18×3、注入 A/B、
沙箱对照、端到端链路。README 第一屏改成「链路图 → 实测结论 → 不信任模型输出 →
快速开始」——**安装步骤不该排在论点前面**。
...
**四个原定任务全部完成。** 下面是按「对秋招的边际收益」排的候选，不是必做清单。

1. **换一个模型再跑一轮评测**（Claude 或 GPT，18×3）。
   这是现在**性价比最高**的一件事：所有结论目前都挂在"只测过 DeepSeek-v4-pro"
   这一个前提上，多一个模型就能把"我测过"变成"我比较过"。
   成本估计：Claude Sonnet 约 $3～5，DeepSeek 约 $0.4。
2. **防住 `file_content` 那条注入向量**。现在挡不住（A/B 里它纹丝不动，
   是刻意留的诚实缺口）。做了就能说"两条向量都覆盖"。
   思路：工具结果回传给 LLM 时也过一遍围栏，并在 system 里声明工具输出是数据。
3. **容器镜像按目标仓库的 requirements 装依赖**。现在只预装了 pytest，
   所以只接得住零依赖仓库。⚠️ 难点是**装依赖那一步本身也在跑别人的代码**
   （`setup.py` / build hook），需要一个单独的、联网但更受限的构建阶段。
4. 小的：API key 落库存哈希 + 限流；trace 接 OTLP/Jaeger；
   clone 缓存的跨进程文件锁与容量上限；webhook「登记投递 + 入队」并进一个事务。

> ⏸️ **秋招期间的建议：先停在这里。** 项目已经有完整链路 + 可量化结论 +
> 三份实验对照，`docs/resume.md` 也写好了。**再加功能的边际收益低于
> 「把已有的东西讲熟」** —— 面试考的是能不能讲清楚，不是行数。
