...
- [四个必须说对的实现细节](#四个必须说对的实现细节)
- [这一层为什么这么薄（面试的落点）](#这一层为什么这么薄面试的落点)

**Why a coding agent needs a sandbox**

**Prompt 注入：不信任模型的**输入****

- [攻击面在哪（不是假想的，是这个项目真实存在过的洞）](#攻击面在哪不是假想的是这个项目真实存在过的洞)
- [分层防御（一层都不够，得叠）](#分层防御一层都不够得叠)
- [「防住了注入」怎么判分（这是设计上最难的一步）](#防住了注入怎么判分这是设计上最难的一步)
- [审计日志的两个设计点（Java 那边的直觉直接能用）](#审计日志的两个设计点java-那边的直觉直接能用)
- [怎么证明防护有效（先红后绿）](#怎么证明防护有效先红后绿)

**Token 计量与成本控制：一个 run 到底花了多少钱**

- [插桩点只有一个（这是设计带来的，不是运气）](#插桩点只有一个这是设计带来的不是运气)
- [★四个 token 字段，不是两个](#四个-token-字段不是两个)
- [★「不知道」不能报成 0](#不知道不能报成-0)
- [★成本的分母是「修对的数量」，不是「总数」](#成本的分母是修对的数量不是总数)
- [per-run 累加的前提，以及怎么钉住它](#per-run-累加的前提以及怎么钉住它)
- [Prompt caching：算完发现开了是纯亏](#prompt-caching算完发现开了是纯亏)
- [落库：加字段即落库，不用改 schema](#落库加字段即落库不用改-schema)

**OpenTelemetry：日志之外，还需要知道时间花在哪**

- [为什么埋点代码里一处 `if enabled:` 都没有](#为什么埋点代码里一处-if-enabled:-都没有)
- [埋在哪：一处包住 N 个](#埋在哪一处包住-n-个)
- [★吞异常的地方，trace 会跟着一起骗你](#吞异常的地方trace-会跟着一起骗你)
- [父子关系不用手工传（这是 Python 的便宜）](#父子关系不用手工传这是-python-的便宜)
- [什么时候**不该**把两段串成一条 trace](#什么时候不该把两段串成一条-trace)
- [三个会咬人的默认值](#三个会咬人的默认值)
- [span 属性是明文，而且会导出到别人家](#span-属性是明文而且会导出到别人家)
- [怎么测 trace](#怎么测-trace)

**换一个模型供应商：单方法协议的第三次兑现**

- [OpenAI 兼容协议 vs Anthropic 协议：四处真实差异](#openai-兼容协议-vs-anthropic-协议四处真实差异)
- [★把别人的 usage 字段映射到自己的桶，不是逐字段改名](#把别人的-usage-字段映射到自己的桶不是逐字段改名)
- [平价表撞上时间相关定价](#平价表撞上时间相关定价)
- [配置的默认值要跟着相关配置走](#配置的默认值要跟着相关配置走)
- [配置里最危险的失败形态：静默忽略](#配置里最危险的失败形态静默忽略)

**评测跑一轮等于报运气**

- [报三个数，不报一个](#报三个数不报一个)
- [怎么飘的比飘多少更有信息](#怎么飘的比飘多少更有信息)
- [按轮跑，不是按 case 连跑](#按轮跑不是按-case-连跑)
- [实现上的两个小决定](#实现上的两个小决定)

**clone 目标仓库：从「我们的仓库」到「别人的仓库」**

- [★把慢操作从 webhook 里挪出去](#把慢操作从-webhook-里挪出去)
- [★不加那一列数据库字段](#不加那一列数据库字段)
- [★仓库名同时是 URL 和路径](#仓库名同时是-url-和路径)
- [★发凭证不是免费的（实测踩的坑）](#发凭证不是免费的实测踩的坑)
- [★凭证放哪：argv / 磁盘 / 环境变量](#凭证放哪argv-磁盘-环境变量)
- [★`copytree` 默认会跟着符号链接走](#copytree-默认会跟着符号链接走)
- [★把注释里的安全约定变成代码里的闸门](#把注释里的安全约定变成代码里的闸门)
- [缓存要么完整要么不存在](#缓存要么完整要么不存在)
- [测试里的"远端"该有多真](#测试里的远端该有多真)

**API 鉴权：默认拒绝，以及权限模型要长得像业务约束**

- [★认证方案要配得上威胁模型](#认证方案要配得上威胁模型)
- [★默认拒绝：挂在 router 上，不是逐个路由加](#默认拒绝挂在-router-上不是逐个路由加)
- [★权限模型要长得像业务约束](#权限模型要长得像业务约束)
- [★审计字段不能由被审计者提供](#审计字段不能由被审计者提供)
- [401 和 403 不是一回事](#401-和-403-不是一回事)
- [★401 的响应体要对外统一](#401-的响应体要对外统一)
- [fail closed，但要喊出来](#fail-closed但要喊出来)
- [常数时间比较，而且不要短路](#常数时间比较而且不要短路)
- [这一步没做的事（面试要主动说）](#这一步没做的事面试要主动说)
...
做完这一步之前，webhook 那条路验签严得很（HMAC + 常数时间比较 + fail closed），
而**自己的 HTTP 接口是裸的** —— `POST /runs` 和 `POST /approval` 谁都能调。
一个项目里两条入口的安全水位差这么多，本身就是个信号。

### ★认证方案要配得上威胁模型

**刻意不做 OAuth。** 调用方是 CI 机器人和少数几个人，不是"任意第三方应用
代表用户访问资源" —— 那才是 OAuth 存在的理由（它解决的是**授权委托**，
不是"证明你是谁"）。用 OAuth 只会引入一个授权服务器、一堆回调、
和一个我在面试里讲不清楚的登录流程。

> **安全方案选重了不叫稳妥，叫没想清楚。** 面试被问"为什么不上 OAuth/JWT"，
> 能说清"我的威胁模型是什么、这个方案覆盖到哪"，比堆名词有用得多。
>
> 顺带：**JWT 在这里也是负分**。JWT 的卖点是无状态校验（不用查库），
> 代价是**吊销很难**（签出去就有效到过期）。我只有几个调用方，
> 查一次内存里的列表零成本，却白白换来一个撤不掉的凭证。

### ★默认拒绝：挂在 router 上，不是逐个路由加

```python
public = APIRouter()                                        # 只有 /health 和 webhook
router = APIRouter(dependencies=[Depends(require_scope("run"))])   # 其余全部
```

逐个路由加 `Depends` 也能跑，但**失败形态完全不同**：

| 写法 | 忘了写会怎样 |
|---|---|
| 逐个路由加 | 新接口**是公开的**，不报错、不告警，直到被人发现 |
| 挂在 router 上 | 新接口**默认受保护**；要公开必须显式挪进 `public` |

> **一个安全控制如果靠人记得加，它迟早会漏。** 判断标准是「忘了做会不会
> 有人告诉你」—— 会报错的疏忽不可怕，**静默生效的疏忽才可怕**。
>
> Java 对照：Spring Security 写 `anyRequest().authenticated()` +
> 显式 `permitAll()` 白名单，而不是满代码撒 `@PreAuthorize`。同一个道理。

配套的测试也**不手写接口清单**，而是遍历 `app.openapi()["paths"]` 挨个打一遍
匿名请求。手写的清单会跟着新接口一起过期，而**过期的安全测试比没有测试更糟** ——
它给你一个"测过了"的错觉。

### ★权限模型要长得像业务约束

整个项目的核心论点是「Agent 说成功不算数，要人批准」。那么：

> **如果开 run 的那把 key 也能批准自己开的 run，这道闸门就是装饰品。**

所以 `approve` 是**单独**的权限位，只挂在审批那一个接口上。
CI 机器人拿 `run`，人拿 `run,approve`。

这不是"顺便做个 RBAC 显得正规"。**权限的切分线应该落在业务约束上**，
不是落在"读/写"这种技术分类上。读写分权在这里毫无意义 —— 开 run 也是写。

> 一般化：**职责分离（separation of duties）**。会计里"记账的人不能审批付款"，
> CI 里"部署的人不能批准自己的 PR"，是同一个东西。

### ★审计字段不能由被审计者提供

以前 `decided_by` 是**请求体里的一个字段**：

```json
{"decision": "approved", "decided_by": "the CTO"}
```

也就是说审批流水上"谁批的"是**被审计的人自己填的**。那不是审计，是留言板。
现在它取自认证出来的 `Principal.name`，请求体里的同名字段被无视。实测：

```
POST /approval  body 里自称 "the CTO"   → 200
GET  /approvals                         → "decided_by": "kayou"
```

`reason` 反过来**就该**由人填 —— 它是主观说明，不是身份断言。
**区分「事实」和「说法」：事实由服务端从已验证的上下文取，说法才收用户输入。**

### 401 和 403 不是一回事

| | 含义 | 调用方该做什么 |
|---|---|---|
| **401** Unauthorized | 你没表明身份（或身份不认识） | 去拿一把 key |
| **403** Forbidden | 我知道你是谁，但你不能干这个 | 去要权限，换 key 没用 |

混在一起的后果很具体：回 401 会让调用方以为 key 坏了，
**跑去重新签发一把同样没权限的**。

两个细节：
- 401 **必须带 `WWW-Authenticate` 头**（RFC 9110），客户端库靠它知道用哪种认证重试。
  这也是没用 fastapi 自带 `HTTPBearer` 的原因 —— 它缺 header 时抛的是 403。
- 403 的响应体可以**明说缺哪个权限**（`它有：run`）。权限名不是秘密，
  而让人猜"我到底缺什么"纯属浪费时间。

### ★401 的响应体要对外统一

"服务端一把 key 都没配"和"你这把 key 不对"，对外必须是**同一个 401、
同一句话**。不然 401 本身就成了一个探测接口 —— 攻击者一眼看出
"这台服务器压根没配 key"。详细原因只进日志。

> **报错要对运维详细、对外部统一。** 同一条原则的另外两个化身：
> 登录失败别说"用户名不存在"（那是用户枚举）；
> 500 别把堆栈吐给客户端。

### fail closed，但要喊出来

一把 key 都没配 = 拒绝所有，不是"没配就不鉴权"。和 `github_webhook_secret`
同一条规矩：**默认放行的开关是最典型的生产事故**（某次部署漏注入一个环境变量，
接口就裸奔了，日志里一条报错都没有）。

但**沉默地 fail closed 会让人对着一片 401 查半天**，所以启动时打一条
WARNING 把原因和解法都说清。

> **"安全"和"可诊断"不是对立的。** 让对的行为容易理解，人才不会去关掉它。
...
for configured in settings.api_keys:
    if hmac.compare_digest(configured.key.encode(), token.encode()):
        found = configured      # ← 刻意不 return
return found
```

两层：
1. `==` 一发现某字节不同就返回，耗时泄露"前几位对上了"，攻击者逐字节能把
   key 试出来。和 webhook 验签同一个坑，同样用 `hmac.compare_digest`
   （Java：`MessageDigest.isEqual`）。
2. 循环里 `if match: return` 会从耗时里泄露**匹配到第几把 key**。
   这条弱得多，但代价只是少写一个 `return` —— **不值得省的安全性别省**。

### 这一步没做的事（面试要主动说）

- **key 明文存在配置里**，没有轮转、没有吊销列表、没有"上次使用时间"。
  真要做得落库存哈希（像存密码那样），泄露一份配置就不等于泄露所有 key。
- **没有限流。** 一把泄露的 key 可以无限开 run —— 而每个 run 都在烧 token。
  成本熔断是 per-run 的，拦不住"开一万个 run"。
- **浏览器的 `EventSource` 设不了 Authorization 头**，所以 SSE 那个接口目前
  只有 curl / `fetch` 能用。要支持浏览器得发一个短期一次性 token 走 query，
  而那会把 token 漏进访问日志 —— 这是个真实的取舍，不是遗漏。
