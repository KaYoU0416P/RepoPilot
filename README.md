...
| 结论 | 数字 | 怎么复现 |
|---|---|---|
| Agent 修 bug 的**可靠**成功率 | **89%**（平均 93%） | `make bench ARGS="--repeat 3"`（[存档](docs/evidence/bench-18x3.md)） |
| 平均修对一个 bug 的花费 | **$0.0080** | 同上，报表里的「★平均修对一个」 |
| 注入防护**是否真的起作用** | 关掉后载荷落地 **0/9 → 4/9** | `uv run python scripts/ab_injection.py` |
| 端到端链路（含审批闸门） | 热启动 **4 秒**跑完 | `make demo-flow`（[存档](docs/evidence/end-to-end-flow.md)） |
| 容器沙箱**是否真的隔离** | 越狱项 **4 → 0** | `make sandbox-check` vs `ARGS=--local`（[存档](docs/evidence/sandbox-isolation.md)） |

「可靠成功率」= **每一轮都对才算**。接进 CI 要的不是"平均能修对"，
是"这个 case 会不会**稳定**修对" —— 你没法把抛硬币的东西放进流水线。
⚠️ 只测过一个模型（DeepSeek-v4-pro）、3 轮，够看稳定性，不够给置信区间。
